...
-(void) peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    for (CBCharacteristic *c in service.characteristics) {
        [self updateLog:[NSString stringWithFormat:@"特征 UUID: %@ (%@)",c.UUID.data,c.UUID]];
                if ([c.UUID isEqual:[CBUUID UUIDWithString:@"FF38"]]) {
                    NSLog(@"%@  *****************%@",c.descriptors.description,c.description);
        ...
        NSLog(@"特征(%lu) UUID: %@ (%@)",(unsigned long)c.properties,c.UUID.data,c.UUID);
...
{
    NSLog(@"33333333%@====",characteristic.value);
...
//            {
//                [_labelone setTitle:[NSString stringWithFormat:@"%@",c.value] forState:UIControlStateNormal];
//                
//            }
//        }
    }
...
-(void)open {
    for (CBCharacteristic *c in _nCharacteristics) {
        ...
        if ([c.UUID isEqual:[CBUUID UUIDWithString:@"FFF4"]]) {
            _writeCharacteristic = c;
            [_labelone setTitle:[NSString stringWithFormat:@"%@",c.UUID] forState:UIControlStateNormal];
            ...
            NSLog(@"333特征 UUID: %@ (%@)",c.UUID.data,c.value);
